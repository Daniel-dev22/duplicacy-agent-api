...
- **Per-node Traefik label** at `duplicacy-api.{hostname-short}.{domain}` — same pattern as `portainer-agent.{node}.{domain}`. K8s side has one ExternalName Service per node so the central router can address agents by stable name.
- **Push/pull split**:
...
| `CONFIG_DIR` | `/var/lib/duplicacy-api` | Persistent dir (events.sqlite, schedule cache, filter cache) |
...
ansible-playbook /home/user/ansible/kubernetes/build-deploy.yml \
  -e project=duplicacy-api -e target_servers=kd-nas01,ng-nas01
```

Local development:

```bash
cd /srv/containers/duplicacy-api
go build ./...
```
