...
//   - Application-layer per-node identity is conveyed via the bearer token
//     attached automatically by agent-kit-go/transport's bearer round-tripper.
//
// The pooled client itself is now built by agent-kit-go/transport
// (NewRouterClient), the DRY promotion of this agent's and gdrive's
// byte-identical client builders. Pool/dial knobs are the kit's canonical
// values; see buildControlCenterClient for the convergence note.
...
import (
	...
	"github.com/Daniel-dev22/agent-kit-go/transport"
)
...
// buildControlCenterClient returns the pooled HTTP client, built by the shared
// agent-kit-go/transport package. Two operating modes (unchanged):
//
...
//     and forwards into the cluster. Traefik is re-dialed on every connect so
//     a Traefik restart is picked up without an agent restart. The bearer
//     token is attached automatically to every request lacking an
//     Authorization header.
//
// Pool-knob convergence (kit adopts gdrive's canonical values): this agent
// previously used Dialer.KeepAlive 15s and IdleConnTimeout 0 (unbounded), and
// left ForceAttemptHTTP2 unset. The kit's canonical values are KeepAlive 30s,
// IdleConnTimeout 90s, ForceAttemptHTTP2 true. This is a deliberate,
// functionally-safe convergence for a client dialing the local Traefik —
// bounding idle connections at 90s is strictly better than never reaping them.
// The error return is retained for call-site compatibility; it is always nil.
func buildControlCenterClient(cfg Config) (*http.Client, error) {
	return transport.NewRouterClient(transport.RouterClientConfig{
		TraefikDockerDNS: cfg.TraefikDockerDNS,
		TraefikDialPort:  cfg.TraefikDialPort,
